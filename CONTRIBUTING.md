...
3. Publish with `yarn workspace @i-vresse/wb-form publish`
4. Create git tag for version with `git tag @i-vresse/wb-form@${packages/form/package.json:version}` and `git push origin --tags`
...
2. Add changes to [CHANGELOG.md](CHANGELOG.md) and push changes to main branch
3. Publish with `yarn workspace @i-vresse/wb-core publish`
4. Create git tag for version with `git tag @i-vresse/wb-core@${packages/core/package.json:version}` and `git push origin --tags`
